...
var config   = window[CANVAS_SWF + "Options"] || {};
var BASE_URL = config["swfPath"] || getScriptUrl().replace(/[^\/]+$/, "");
var SWF_URL  = BASE_URL + "canvas.swf";
...
function onReadyStateChange() {
    if (document.readyState === "complete") {
        ...
        for (var i = 0, n = canvases.length; i < n; ++i) {
            CanvasSwf.initElement(canvases[i]);
        }
    }
}
...
// Get the absolute URL of canvas.swf.js
